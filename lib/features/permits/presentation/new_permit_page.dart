...
class _NewPermitPageState extends ConsumerState<NewPermitPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final types = sectorId == null ? null : ref.watch(undertakingTypesProvider(sectorId!));
    return Scaffold(appBar: AppBar(title: const Text('New Permit')), body: Form(key: formKey, child: ListView(padding: const EdgeInsets.all(20), children: [
      ...
      if (types != null) types.when(loading: () => const LinearProgressIndicator(), error: (e, _) => Text('Unable to load undertaking types: $e'), data: (items) => DropdownButtonFormField<String>(initialValue: undertakingTypeId, decoration: const InputDecoration(labelText: 'Undertaking type', border: OutlineInputBorder()), items: items.map<DropdownMenuItem<String>>((t) => DropdownMenuItem<String>(value: t.id, child: Text(t.name))).toList(), onChanged: (v) => setState(() => undertakingTypeId = v), validator: (v) => v == null ? 'Select an undertaking type' : null)) else const SizedBox.shrink(),
