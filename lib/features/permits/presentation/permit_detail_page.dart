...
class PermitDetailPage extends ConsumerWidget {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Unable to load permit: $error', textAlign: TextAlign.center),
                const SizedBox(height: 12),
                FilledButton(
        ...
        data: (p) => ListView(
          ...
          children: [
            ...
            _group(context, 'Proponent', [
              ...
              _row('Contact person', p.contactPerson),
              _row('Phone', p.contactPhone),
            ]),
            _group(context, 'Location', [
              _row('Region', p.region),
              _row('District', p.district),
              _row('Town / site', p.townSite),
              _row('Latitude', p.latitude?.toString()),
              _row('Longitude', p.longitude?.toString()),
            ]),
            ...
            _group(context, 'Validity', [
              ...
              _row('Validity', p.validityMonths == null ? null : '${p.validityMonths} months'),
              _row('Expiry date', _date(p.expiryDate)),
            ]),
            _group(context, 'Fees', [
              _row('Permit fee', p.permitFee.toStringAsFixed(2)),
              _row('Processing fee', p.processingFee.toStringAsFixed(2)),
              _row('Total cost', p.totalCost.toStringAsFixed(2)),
            ]),
            if (p.revoked)
              const Card(
                child: ListTile(
  ...
  Widget _group(BuildContext context, String title, List<Widget> children) => Card(
  ...
  Widget _row(String label, String? value) => Padding(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(width: 125, child: Text(label, style: const TextStyle(fontWeight: FontWeight.w600))),
